...
function toggleMarkerManager() {
  ...
  if (mgr) {
    if (showMarketManager) {
      mgr.addMarkers(markers.countries, 0, 10);
      mgr.addMarkers(markers.places, 11, 20);
      mgr.addMarkers(markers.locations, 21);
      mgr.refresh();
    } else {
      mgr.clearMarkers();
      mgr.refresh();
...
function initialize() {
  ...
  map = new google.maps.Map(document.getElementById('map'), {
    center: new google.maps.LatLng(28.6088333, -106.0341733),
    zoom:12,
    mapTypeId: 'terrain'
  });
...
/*
  new google.maps.Marker({
        ...
        , map: map
        , title: 'Titulo fnc'
        //, icon: 'http://localhost/mf45/images/delphi.png'
        , cursor: 'default'
        , draggable: false
    });
*/
